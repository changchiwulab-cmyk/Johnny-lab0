...
class ContractRecommendationsGenerator {
  ...
  async generate(risks, compliance, terms) {
    ...
    try {
      const recommendations = {
        ...
        negotiation_points: this.generateNegotiationPoints(risks, compliance, terms),
        compliance_improvements: this.generateComplianceImprovements(compliance),
      ...
      recommendations.metadata.critical_actions = recommendations.critical_actions.filter(
  ...
  generateExecutiveSummary(risks, compliance) {
    const totalIssues = (risks.metadata?.total_risks || 0) + (compliance.metadata?.total_issues || 0);
    const criticalCount = (risks.metadata?.high_severity || 0) + (compliance.metadata?.critical || 0);
    ...
    const lowCount = (risks.metadata?.low_severity || 0) + (compliance.metadata?.low || 0);
    if (lowCount > 0) {
...
if (require.main === module) {
  ...
  generator.generate(risks, compliance, terms).then((result) => {
    ...
    // 保存到文件
    const outputFile = path.join(path.dirname(risksPath), 'recommendations.json');
