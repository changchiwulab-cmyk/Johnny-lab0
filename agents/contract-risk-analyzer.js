...
class ContractRiskAnalyzer {
  ...
  analyzeLiabilityCap(terms, contractType) {
    ...
    const industry_standard = this.getIndustryStandard(
      contractType,
      "liability_cap",
    );
  ...
  analyzeNDASpecificRisks(terms) {
    ...
    if (
      !terms.key_terms?.permitted_use?.description ||
      !terms.key_terms.permitted_use.description.includes("Limited")
    ) {
      risks.push({
        risk_id: "RISK-010",
  ...
  analyzePurchaseSpecificRisks(terms) {
    ...
    if (
      !terms.key_terms?.warranty?.description ||
      terms.key_terms.warranty.description.includes("No")
    ) {
      risks.push({
        risk_id: "RISK-014",
  ...
  getIndustryStandard(contractType, parameter) {
    ...
    return (
      contractStandards[parameter] || standards.Default[parameter] || 1000000
    );
  }
