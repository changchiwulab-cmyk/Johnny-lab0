...
class ContractComplianceChecker {
  ...
  checkDataProtectionCompliance(terms, jurisdiction) {
    ...
    if (jurisdiction === "California" || jurisdiction === "USA") {
      if (
        !text.match(/CCPA|california consumer privacy|數據隱私|privacy notice/i)
      ) {
        issues.push({
          check_id: "COMP-001",
          ...
          status: "MISSING",
          description:
            "California Consumer Privacy Act (CCPA) 要求在合同中明確說明數據處理",
    ...
    if (jurisdiction.includes("EU") || jurisdiction.includes("Europe")) {
      if (
        !text.match(
          /GDPR|general data protection|data processing|data controller|data processor/i,
        )
      ) {
        issues.push({
          check_id: "COMP-002",
          ...
          status: "MISSING",
          description:
            "GDPR 要求在涉及歐盟個人數據的合同中明確數據控制和處理的角色",
  ...
  checkLiabilityCompliance(terms) {
    ...
    if (
      !text.match(
        /consequential damages|indirect damages|lost profits|loss of data/i,
      )
    ) {
      issues.push({
        check_id: "COMP-005",
  ...
  checkIPCompliance(terms) {
    ...
    if (
      !text.match(
        /intellectual property|IP|patent|trademark|copyright|proprietary/i,
      )
    ) {
      issues.push({
        check_id: "COMP-008",
  ...
  checkDisputeResolutionCompliance(terms) {
    ...
    if (
      !text.match(
        /dispute|arbitration|mediation|jurisdiction|governing law|court/i,
      )
    ) {
      issues.push({
        check_id: "COMP-010",
